...
def main() -> None:
    parser = argparse.ArgumentParser(description="Check GitHub repository settings")
    ...
    if envs and "error" in envs[0]:
        ...
        for env in envs:
            print(f"  - {env['name']}: {env['protection_rules']} protection rules, admin bypass: {env['can_admins_bypass']}")
